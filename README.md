...
1. Criar arquivo em `src/algorithms/novo_algoritmo.py`:

```python
def novo_algoritmo(arr: list[int]) -> tuple[list[int], dict]:
    comparisons = 0
    swaps = 0
...
2. Adicionar no `src/main.py`:

```python
from algorithms import novo_algoritmo

AVAILABLE_ALGORITHMS = {
    # ... outros
    'novo_algoritmo': novo_algoritmo,
}
```
...
Consulte [docs/README.md](docs/README.md) para documentação detalhada sobre:
- Descrição completa de cada algoritmo
